 ...
 * Endpoint: GET /v1/svm/transactions/{address}
 */
export async function fetchWalletTransactions(address, limit = 100) {
  ...
  return res.json(); // { transactions: [...], next_offset: "..." }
}

/**
 * Fetch wallet token balances from Dune SIM
 * Endpoint: GET /v1/svm/balances/{address}
 */
...
/**
 * Extract DEX swap transactions from raw SIM response
 * Returns only transactions that involve DEX programs
 */
...
/**
 * Parse token delta from a single swap transaction
 * Returns { inputMint, outputMint, inputAmount, outputAmount, slippagePct }
 */
export function parseSwapDelta(tx) {
  ...
  // Find the token that DECREASED (sold) and the one that INCREASED (bought)
  const deltas = [];
  ...
  // Estimate slippage vs expected (assume 0.3% normal DEX fee)
  // Any excess beyond 1% is suspicious — potential sandwich victim
  const expectedOutput  = Math.abs(sold.delta) * 0.997; // deducting normal DEX fee only
